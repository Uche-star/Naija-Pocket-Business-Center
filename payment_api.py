...
def init_db() -> None:
    with connect_db() as conn:
        ...
        columns = {row["name"] for row in conn.execute("PRAGMA table_info(payment_orders)").fetchall()}
        if "document_saved_path" not in columns:
            conn.execute("ALTER TABLE payment_orders ADD COLUMN document_saved_path TEXT")
        if "document_saved_at" not in columns:
            conn.execute("ALTER TABLE payment_orders ADD COLUMN document_saved_at TEXT")
        conn.commit()
...
def saved_document_path(payment: dict[str, Any]) -> Path | None:
    raw = clean(payment.get("document_saved_path"))
    if not raw:
        return None
    path = Path(raw)
...
def saved_document_exists(payment: dict[str, Any]) -> bool:
    path = saved_document_path(payment)
    return bool(path and path.is_file() and path.stat().st_size > 0)


def save_exact_document_snapshot(payment: dict[str, Any], document: dict[str, Any]) -> tuple[bool, str]:
    if saved_document_exists(payment):
        return True, str(saved_document_path(payment))
    pages = normalize_pages(document.get("pages"))
    if not pages and clean(document.get("document_text")):
        pages = [clean(document.get("document_text"))]
    if not pages:
        return False, "The reviewed document contains no downloadable content."
    filename = clean(document.get("filename")) or clean(payment.get("document_filename")) or f"naija_pocket_{payment.get('job_id','document')}.docx"
    if not filename.lower().endswith(".docx"):
        filename += ".docx"
    folder = DOWNLOAD_DIR / clean(payment.get("payment_id"))
    folder.mkdir(parents=True, exist_ok=True)
    output = folder / Path(filename).name
    try:
        generated = make_docx(pages, str(output))
        generated = Path(generated)
        if generated != output:
            if output.exists(): output.unlink()
            generated.replace(output)
        if not output.is_file() or output.stat().st_size <= 0:
            return False, "The reviewed document snapshot was not saved correctly."
        ts=now_iso()
        with connect_db() as conn:
            conn.execute("UPDATE payment_orders SET document_saved_path=?, document_saved_at=?, document_filename=?, updated_at=? WHERE payment_id=?", (str(output),ts,Path(filename).name,ts,clean(payment.get("payment_id"))))
            conn.commit()
...
async def payment_create(request: Request, job_id: str | None = None, customer_id: str | None = None, service: str | None = None, amount: float | None = None, payment_method: str | None = None):
    try:
        body = await request.json()
        if not isinstance(body, dict): body = {}
    except Exception:
        body = {}
    job_id=clean(job_id or body.get("job_id")); customer_id=clean(customer_id or body.get("customer_id")); service=clean(service or body.get("service")); method=clean(payment_method or body.get("payment_method")) or DEFAULT_PAYMENT_METHOD
    if amount is None: amount=body.get("amount")
    if not job_id: return json_response_error("JOB_ID_REQUIRED","job_id is required.",400)
    current,error=safe_current_document(job_id)
    if error or current is None: return json_response_error("DOCUMENT_LOOKUP_FAILED","The existing document service could not be reached for this job.",502,detail=error)
    requested_version=clean(body.get("version_id") or body.get("document_version") or body.get("versionId"))
    supplied_pages=body.get("pages", body.get("document_pages"))
    supplied_text=clean(body.get("document_text") or body.get("text") or body.get("content"))
    supplied_filename=clean(body.get("filename") or body.get("document_filename"))
    exact=bool(requested_version and (supplied_pages is not None or supplied_text))
    backend_version=clean(current.get("version_id"))
    if exact and backend_version and backend_version != requested_version:
        return json_response_error("PAYMENT_DOCUMENT_CHANGED","The reviewed document version no longer matches the current job version. Please return to Review and try again.",409,job_id=job_id,version_id=requested_version,current_version=backend_version)
    if exact:
        doc={"job_id":job_id,"pages":supplied_pages if supplied_pages is not None else [],"document_text":supplied_text,"version_id":requested_version,"filename":supplied_filename or clean(current.get("filename")) or f"naija_pocket_{job_id}.docx","service":service or clean(current.get("service")),"customer_id":customer_id or clean(current.get("customer_id")),"amount":money(current.get("amount")),"review_finished":True,"status":"review_complete"}
    else:
        doc=current
        if not current_document_is_ready(doc): return json_response_error("DOCUMENT_NOT_READY","The document is not ready for payment yet.",409,job_id=job_id,status=doc.get("status"))
    if not document_has_content(doc): return json_response_error("DOCUMENT_EMPTY","The reviewed document contains no downloadable content.",409,job_id=job_id)
    version=clean(doc.get("version_id")) or backend_version or requested_version
    if not version: return json_response_error("VERSION_ID_REQUIRED","A document version is required before payment can be created.",409,job_id=job_id)
    final_amount=money(doc.get("amount")) or money(amount)
    if final_amount<=0: return json_response_error("AMOUNT_NOT_AVAILABLE","The document service did not provide an amount to pay.",409,job_id=job_id)
    latest=get_latest_payment_for_job(job_id)
    if latest and clean(latest.get("document_version"))==version and normalize_status(latest.get("payment_status")) in {"pending","reported","verification_pending","awaiting_verification","verified","completed","complete","paid"}:
        if not saved_document_exists(latest):
            ok,err=save_exact_document_snapshot(latest,doc)
            if not ok: return json_response_error("DOCUMENT_SNAPSHOT_FAILED","The exact reviewed document could not be saved before payment.",500,detail=err,job_id=job_id,version_id=version)
            latest=get_payment(latest["payment_id"]) or latest
        return {"ok":True,"message":"Existing payment and exact reviewed document snapshot reused.","payment":payment_public(latest),"payment_id":latest["payment_id"],"amount":money(latest["amount"]),"currency":latest["currency"],"payment_status":latest["payment_status"],"paid":payment_is_verified(latest["payment_status"]),"payment_verified":payment_is_verified(latest["payment_status"]),"download_unlocked":payment_is_verified(latest["payment_status"]),"document_version":version,"version_id":version,"document_saved":True}
    payment_id=f"NPB-{uuid.uuid4().hex[:12].upper()}"
    payload=snapshot_payload(doc); payload.update({"version_id":version,"document_version":version,"customer_id":customer_id or clean(doc.get("customer_id")),"service":service or clean(doc.get("service")),"amount":final_amount,"currency":DEFAULT_CURRENCY})
    record=create_payment_record(payment_id=payment_id,job_id=job_id,customer_id=customer_id or clean(doc.get("customer_id")),service=service or clean(doc.get("service")),amount=final_amount,currency=DEFAULT_CURRENCY,payment_method=method,document_version=version,document_filename=supplied_filename or clean(doc.get("filename")) or f"naija_pocket_{job_id}.docx",document_payload=payload)
    ok,err=save_exact_document_snapshot(record,doc)
    if not ok:
        with connect_db() as conn: conn.execute("DELETE FROM payment_orders WHERE payment_id=?",(payment_id,)); conn.commit()
        return json_response_error("DOCUMENT_SNAPSHOT_FAILED","The exact reviewed document could not be saved. Payment has not been started.",500,detail=err,job_id=job_id,version_id=version)
    record=get_payment(payment_id) or record
    return {"ok":True,"message":"Payment created and the exact reviewed document was saved successfully.","payment":payment_public(record),"payment_id":payment_id,"amount":final_amount,"currency":DEFAULT_CURRENCY,"payment_status":"pending","paid":False,"payment_verified":False,"download_unlocked":False,"document_version":version,"version_id":version,"document_saved":True}
...
async def customer_care_verify(
    ...
    if not saved_document_exists(payment):
        return json_response_error("DOCUMENT_SNAPSHOT_MISSING","Customer Care cannot unlock this payment because the exact document snapshot is missing.",409,payment_id=payment_id)

    current, error = safe_current_document(payment["job_id"])
    if error or current is None:
        return json_response_error("DOCUMENT_LOOKUP_FAILED","Customer Care verification cannot be completed because the current document could not be checked.",502,detail=error)
...
async def download_document(payment_id: str | None = None, job_id: str | None = None):
    payment=get_payment(clean(payment_id)) if clean(payment_id) else None
    if not payment and clean(job_id): payment=get_latest_payment_for_job(clean(job_id))
    if not payment: return json_response_error("PAYMENT_NOT_FOUND","Payment record not found.",404)
    if not payment_is_verified(payment.get("payment_status","")): return json_response_error("DOWNLOAD_LOCKED","Download remains locked until Customer Care verifies the payment.",403,payment_status=payment.get("payment_status"),payment_id=payment.get("payment_id"),download_unlocked=False)
    output=saved_document_path(payment)
    if not output or not output.is_file() or output.stat().st_size<=0: return json_response_error("DOCUMENT_SNAPSHOT_MISSING","The paid document snapshot is missing. Download remains locked for safety.",409,payment_id=payment.get("payment_id"),version_id=payment.get("document_version"),download_unlocked=False)
    increment_download(payment["payment_id"])
    filename=clean(payment.get("document_filename")) or output.name
    return FileResponse(path=str(output),media_type="application/vnd.openxmlformats-officedocument.wordprocessingml.document",filename=Path(filename).name,headers={"X-Payment-ID":payment["payment_id"],"X-Document-Version":clean(payment.get("document_version")),"X-Download-Unlocked":"true","X-Document-Snapshot":"true"})
