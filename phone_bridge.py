...
<style>
body { background:#0B0B0B; color:white; font-family:Arial,sans-serif; text-align:center; padding:25px; margin:0; }
.container { max-width:500px; margin:auto; }
.status { background:white; color:black; padding:20px; border-radius:10px; margin:25px 0; }
button { width:100%; padding:16px; margin:8px 0; border:none; border-radius:8px; font-size:16px; font-weight:bold; }
.start { background:#008000; color:white; }
.stop { background:#990000; color:white; }
.play { background:#555555; color:white; }
.again { background:#777777; color:white; }
.send { background:#008000; color:white; }
...
<div class="status"><div id="status">Ready to record your message.</div></div>
<button id="startButton" class="start" onclick="startRecording()">Start Recording</button>
<button id="stopButton" class="stop" onclick="stopRecording()" disabled>Stop Recording</button>
<audio id="audioPlayer" controls></audio>
<button id="playButton" class="play" onclick="playRecording()" disabled>Play Recording</button>
<button id="againButton" class="again" onclick="recordAgain()" disabled>Record Again</button>
<button id="sendButton" class="send" onclick="sendToAda()" disabled>Send Recording to Ada</button>
<div id="result"></div>
</div>
<script>
...
async function startRecording() {
    try {
        ...
        mediaRecorder.onstop = function() {
            ...
            recordedBlob = new Blob(audioChunks, { type:finalType });
...
async function sendToAda() {
    ...
    formData.append("audio", recordedBlob, "ada_voice_message." + extension);
...
def upload_audio():
    try:
        ...
        extension = "mp4" if original_name.endswith(".mp4") else "webm"

        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        unique_id = uuid.uuid4().hex[:8]

        filename = (
            "ada_voice_" + timestamp + "_" +
            unique_id + "." + extension
        )
        ...
        if not os.path.exists(file_path):
            raise RuntimeError("Recording could not be saved.")

        file_size = os.path.getsize(file_path)

        if file_size <= 0:
            os.remove(file_path)
            raise RuntimeError("The received recording was empty.")
