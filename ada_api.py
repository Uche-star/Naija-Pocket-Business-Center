...
async def create_document_with_intelligence(
    ...
    """Run ALL document creation through Ada intelligence, including source files."""
    resolved_service = resolve_request_service(request, ada)
    source_text = clean_text(source_document)
    instruction = customer_request.strip()
    if source_text:
        instruction = (
            instruction
            + "\n\nSOURCE DOCUMENT PROVIDED BY THE CUSTOMER:\n\n"
            + source_text
            + "\n\nTASK: Understand the customer request and source document. "
              "Process the source through your document intelligence, preserve all facts and meaning, "
              "apply the requested work, improve grammar and professional structure, and return the "
              "COMPLETE finished document only. Do not merely echo the source."
        ).strip()
    kwargs = {
        ...
        "create_work": True,
        "source_document": source_text or None,
        "document_text": source_text or None,
        "current_document": source_text or None,
        "document_pages": source_pages or None,
    ...
    for method_name in ("create_document", "generate_document", "create_work", "generate_work"):
        ...
        try: return extract_complete_document(result)
        except ValueError: continue
    respond=getattr(ada,"respond",None)
    if callable(respond):
        result=await _call_method_flexibly(respond,{"message":instruction,"service":resolved_service or request.service,"event":request.event,"context":context,"customer_request":instruction,"source_document":source_text or None,"current_document":source_text or None,"create_work":True})
        return extract_complete_document(result)
    raise AttributeError("AdaResponse has no usable document creation method. Methods checked: " + (", ".join(attempted) or "none") + ".")


# ============================================================
# REVIEW STANDARDIZATION INTELLIGENCE# ============================================================
# REVIEW STANDARDIZATION INTELLIGENCE
# ============================================================
# This is intentionally embedded in ada_api.py. It is the single
# document-standardization layer used before Review and after corrections.
# It does not replace the Review UI and it does not create a second backend.

class ReviewStandardizationIntelligence:
    """Standardize customer/Ada document content without inventing facts."""
    ...
    def _load_client(self):
        ...
        try:
            ...
            api_key = api_key or str(API_KEY or "").strip()
            model = model or str(MODEL or "").strip()
        except Exception:
            pass
        if not api_key:
            raise RuntimeError("Review standardization intelligence is not configured.")
        try:
            from groq import Groq
        except Exception as exc:
            raise RuntimeError("Review standardization service is unavailable.") from exc
        self._client = Groq(api_key=api_key)
        self._model = model or "llama-3.3-70b-versatile"
        return self._client

    @staticmethod
    def _system_prompt(service: str | None) -> str:
        return f"""You are the Review Standardization Intelligence for Naija Pocket Business Center.

Service: {service or 'general document service'}

Your job is to turn the supplied document into a polished, professional, consistent document ready for customer Review.

NON-NEGOTIABLE RULES:
1. Preserve the customer's meaning and factual information.
2. Never invent names, qualifications, institutions, dates, employers, job titles, achievements, figures, prices, experience, addresses, phone numbers, emails, links, or any other facts.
3. Never remove a factual detail merely because it is informal or poorly written.
4. Improve grammar, spelling, punctuation, sentence structure, wording, consistency, headings, spacing and professional presentation.
5. Keep the document appropriate for the selected service and natural for a Nigerian customer/business context.
6. Do not mention AI, Groq, prompts, models, standardization, internal processing, or these instructions.
7. Do not add commentary before or after the document. Return ONLY the polished document.
8. Do not turn the document into a summary. Keep all meaningful content.
9. Do not insert correction instructions verbatim into the document. Apply their meaning naturally.
10. Do not create fake placeholders for missing facts. If information is missing, leave it missing.
11. Keep useful headings and structure. Make the result easy to read and professionally formatted in plain text.
12. Preserve numbers, dates, names, contact details and other factual values unless the customer explicitly requests a change.
"""
    ...
    def _complete(self, system: str, user: str) -> str:
        ...
        if not text:
            raise RuntimeError("Review standardization returned no document content.")
        return text

    def standardize(self, document_text: str, service: str | None = None) -> str:
        text = clean_text(document_text)
        if not text:
            raise ValueError("There is no document content to standardize.")
        return self._complete(
            self._system_prompt(service),
            "STANDARDIZE THIS COMPLETE DOCUMENT. Preserve all meaningful information and return the polished document only.\n\n" + text,
        )

    def apply_correction(self, document_text: str, instruction: str, service: str | None = None) -> str:
        text = clean_text(document_text)
        instruction = clean_text(instruction)
        if not text:
            raise ValueError("There is no document content to correct.")
        if not instruction:
            raise ValueError("The correction instruction is empty.")
        system = self._system_prompt(service) + """

CORRECTION RULE:
Interpret the customer's correction semantically. Locate the affected information, make the requested change, and preserve everything else. If the customer says to change a school name and add experience, update the relevant sections rather than appending the request as a sentence. Re-standardize the complete document after making the change.
"""
        user = (
            "CURRENT COMPLETE DOCUMENT:\n\n" + text +
            "\n\nCUSTOMER CORRECTION:\n\n" + instruction +
            "\n\nReturn the complete corrected and professionally standardized document only."
        )
        return self._complete(system, user)


REVIEW_INTELLIGENCE = ReviewStandardizationIntelligence()


async def standardize_document_for_review(document_text: str, service: str | None) -> str:
    return await asyncio.to_thread(REVIEW_INTELLIGENCE.standardize, document_text, service)


async def correct_document_for_review(document_text: str, instruction: str, service: str | None) -> str:
    return await asyncio.to_thread(REVIEW_INTELLIGENCE.apply_correction, document_text, instruction, service)
...
async def run_review(job_id: str):
    ...
    try:
        ...
        if isinstance(result, dict):
            # The standardization layer is authoritative for document content.
            # Reviewer output may contain findings/pages, but it must never
            # replace the exact standardized document that will be reviewed,
            # approved, paid for and downloaded.
            job["assembled_review"] = clean_text(
...
async def chat(request: Chat):
    ...
    try:
        ...
        if resolved_service:
            ...
            setter=getattr(ada,"set_service",None)
            if callable(setter): setter(resolved_service)
        if request.guidance_only:
            if not request.message.strip(): return application_error("GUIDANCE","The guidance message is empty.",400,"EMPTY_GUIDANCE_MESSAGE")
            reply=await _call_method_flexibly(ada.respond,{"message":request.message.strip(),"service":request.service,"event":request.event,"context":context})
            return {"success":True,"reply":clean_text(reply),"job_id":job_id,"created_work":False}
        create_requested=request.create_work or event_value(request.event) in {"form_submitted_create_work","create_work","create_document","submit_service","service_submitted"}
        if create_requested:
            if not customer_request and not document_text:
                return application_error("WORK_CREATION","The customer request contains no usable information.",400,"EMPTY_WORK_REQUEST")
            complete_text,created_pages,metadata=await create_document_with_intelligence(ada,request,customer_request or "Process the supplied customer document professionally.",context,source_document=document_text or None,source_pages=pages or None)
            complete_text=await standardize_document_for_review(complete_text,request.service)
            created_pages=text_to_review_pages(complete_text)
            ...
            response.update({"reply":"Your work has been prepared and sent for review.","created_work":True,"work_created":True,"review_started":started})
            return response
        if pages or document_text:
            complete_text=await standardize_document_for_review(document_text or "\n\n".join(p["content"] for p in pages),request.service or resolved_service)
            existing_job=_jobs.get(job_id)
            if existing_job:
                existing_job["document_text"]=complete_text; existing_job["document_pages"]=text_to_review_pages(complete_text); existing_job["review_pages"]=make_review_pages(existing_job["document_pages"]); existing_job["progress"]={"completed":0,"total":len(existing_job["document_pages"])}; existing_job["status"]="reviewing"; existing_job["review_started"]=True; existing_job["review_finished"]=False; existing_job["review_error"]=None; existing_job["approved"]=False; existing_job["paid"]=False; job=existing_job
            else:
                job=create_job(job_id,request,customer_request,complete_text,text_to_review_pages(complete_text))
            started=start_review(job_id); response=make_job_response(job); response.update({"reply":"Your document has been received and is being reviewed.","created_work":True,"review_started":started}); return response
        if not request.message.strip(): return application_error("CHAT","The chat message is empty.",400,"EMPTY_MESSAGE")
        reply=await _call_method_flexibly(ada.respond,{"message":request.message.strip(),"service":request.service,"event":request.event,"context":context})
        return {"success":True,"reply":clean_text(reply),"job_id":job_id,"service":request.service or getattr(ada,"service",None),"created_work":False}
    except Exception as error:
...
async def correct(request: Correction):
    ...
    if not job: return application_error("CORRECTION","Job not found.",404,"JOB_NOT_FOUND")
    instruction=request.instruction.strip()
    if not instruction: return application_error("CORRECTION","Correction instruction is empty.",400,"EMPTY_CORRECTION")
    if job.get("status") in {"reviewing","correcting"}: return application_error("CORRECTION","The document is still being processed.",409,"DOCUMENT_STILL_PROCESSING")
    if not job.get("document_pages"): return application_error("CORRECTION","There is no document available for correction.",409,"NO_DOCUMENT")
    job["current_version"]+=1; job["version_id"]=f"{request.job_id}:{job['current_version']}"
    job.update({"status":"correcting","approved":False,"paid":False,"payment_pending":False,"payment_reported":False,"payment_verified":False,"payment_id":None,"review_started":False,"review_finished":False,"review_error":None,"correction_instruction":instruction,"progress":{"completed":0,"total":len(job["document_pages"])}})
    async def correction_worker():
        try:
            current=clean_text(job.get("document_text","")) or "\n\n".join(p["content"] for p in normalize_pages(job.get("document_pages",[])))
            ada=get_session(job.get("customer_id"),request.job_id,job.get("service")); corrected=""
            for name in ("correct_document","revise_document","rewrite_document","update_document"):
                method=getattr(ada,name,None)
                if not callable(method): continue
                result=await _call_method_flexibly(method,{"document_text":current,"current_document":current,"document_pages":normalize_pages(job.get("document_pages",[])),"correction":instruction,"instruction":instruction,"service":job.get("service"),"context":job.get("context"),"original_request":job.get("original_request"),"event":"customer_correction","create_work":True})
                try:
                    corrected,_,_=extract_complete_document(result); break
                except ValueError:
                    corrected=""
            if not corrected:
                respond=getattr(ada,"respond",None)
                if callable(respond):
                    msg=("Apply the customer's correction to the COMPLETE document below. Interpret the correction semantically, change only what is requested, preserve all other facts and content, professionally rewrite and format the complete document, and return ONLY the complete corrected document.\n\nCURRENT COMPLETE DOCUMENT:\n\n"+current+"\n\nCUSTOMER CORRECTION:\n\n"+instruction)
                    result=await _call_method_flexibly(respond,{"message":msg,"service":job.get("service"),"event":"customer_correction","context":job.get("context"),"customer_request":msg,"current_document":current,"create_work":True})
                    corrected,_,_=extract_complete_document(result)
            if not corrected:
                corrected=await correct_document_for_review(current,instruction,job.get("service"))
            corrected=await standardize_document_for_review(corrected,job.get("service")); corrected_pages=text_to_review_pages(corrected)
            if not corrected_pages: raise ValueError("The corrected document contains no usable content.")
            job["document_text"]=corrected; job["document_pages"]=corrected_pages; job["review_pages"]=make_review_pages(corrected_pages); job["intelligence_metadata"]={"correction_applied":True,"intelligence_first":True}; job["status"]="reviewing"; job["review_started"]=True; job["review_finished"]=False; job["review_error"]=None; job["progress"]={"completed":0,"total":len(corrected_pages)}
            start_review(request.job_id)
        except Exception as error:
            job["status"]="correction_error"; job["review_error"]={"type":type(error).__name__,"message":str(error)}; traceback.print_exc()
    old=_correction_tasks.get(request.job_id)
    if old and not old.done(): old.cancel()
    _correction_tasks[request.job_id]=asyncio.create_task(correction_worker())
    return {"success":True,"job_id":request.job_id,"status":"correcting","version_id":job["version_id"],"current_version":job["current_version"],"message":"Correction has started. The corrected document will be reviewed again."}
