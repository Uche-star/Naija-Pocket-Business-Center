...
def save_document_to_storage(job_id_str: str) -> dict[str, Any]:
    ...
    if not job:
        raise RuntimeError(f"Job {job_id_str} not available.")
    ...
    job["storage_reference"] = str(filepath)
    job["work_id"] = safe_int(persisted.get("id"), work_id)
...
def recover_saved_job_for_approval(supplied_job_id: str, supplied_version_id: str) -> dict[str, Any] | None:
    ...
    if not filepath.exists():
        ...
        if not document_text:
            document_text = clean_text(persisted_job.get("document_text", ""))
        if not document_text:
            return None
...
def normalize_pages(pages: Any) -> list[dict[str, Any]]:
    if isinstance(pages, list):
        return [{"page_number": i, "position": i, "content": clean_text(p.get("content", p))} for i, p in enumerate(pages, 1) if isinstance(p, dict)]
    return []

def make_review_pages(pages: Any) -> list[dict[str, Any]]:
    return [{"page_number": i, "position": i, "status": "queued", "content": page["content"], "review": "", "error": None} for i, page in enumerate(normalize_pages(pages), 1)]
...
async def run_review(job_id: str):
    ...
    try:
        ...
        job["current_version"] = saved["version"]
        job["version_id"] = saved["version_id"]
        job["storage_reference"] = saved["storage_reference"]
        job["work_id"] = saved["work_id"]
...
def serve_html(filename: str):
    ...
    if not path:
        return application_error("PAGE", "Not found", 404)
...
async def get_review(job_id: str):
    job = _jobs.get(job_id)
    if not job:
        return application_error("REVIEW", "Job not found", 404)
...
async def get_review_pages(job_id: str):
    job = _jobs.get(job_id)
    if not job:
        return application_error("REVIEW_PAGES", "Job not found", 404)
...
async def approve(request: Approval):
    ...
    if not job:
        job = recover_saved_job_for_approval(request.job_id, request.version_id)
    if not job:
        return application_error("APPROVAL", "Job not found", 404)
    job["approved"] = True
    job["status"] = "approved"
...
async def download(work_id: int, version_id: str):
    work = get_activated_work(work_id)
    if not work:
        return application_error("DOWNLOAD", "Not activated", 403)
    ...
    if not filepath.exists():
        ...
        if document_text:
            ...
            return application_error("DOWNLOAD", "File missing", 404)
