...
job_id = re.sub(r"[^A-Za-z0-9_-]", "", str(job.get("job_id") or "job"))
version = re.sub(r"[^A-Za-z0-9-]", "", str(job.get("current_version") or 1))
path = DOWNLOAD_DIR / f"naija_pocket_business{job_id}_v{version}.docx"
...
text = re.sub(r""(?:markdown|md|text)?", "", text, flags=re.I) return text.replace(""", "").strip()

def application_error(stage: str, error: Exception | str, status: int = 500, code: str = "APPLICATION_ERROR"):
print(f"[{stage}] {type(error).name}: {error}")
...
"stage": stage,
"error": code,
"error_type": type(error).name if isinstance(error, Exception) else "ApplicationError",
